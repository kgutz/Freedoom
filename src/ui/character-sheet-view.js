...
export function renderCharacterSheet({ document, state, stats, heroModel }) {
  ...
  root.innerHTML = `<section class="character-identity">
    ...
    <section class="character-attributes-panel"><header><span>ATRIBUTOS</span><b><span class="resource-icon resource-icon--attribute-points" aria-hidden="true"></span>${sheet.availablePoints} PUNTOS DISPONIBLES</b></header><div class="character-attribute-grid">${attributes}<section class="character-derived character-derived--compact"><header><span>ESTADÍSTICAS</span></header><div><b><i>⚔</i><span>${combat.physicalAttack}</span></b><b><i>✦</i><span>${combat.magicAttack}</span></b><b><i>🛡</i><span>${combat.defense}</span></b><b><i>◎</i><span>${(combat.criticalChance*100).toFixed(1)}%</span></b></div></section></div><button type="button" class="character-reset-attributes character-reset-attributes--panel" data-character-reset-attributes ${sheet.spentPoints ? '' : 'disabled'}>RESETEAR ATRIBUTOS</button></section>`;
}

...
describe('interfaz de inventario y botín', () => {
  ...
  it('abre cada comercio con su nombre y descripción contextual', () => {
    ...
    expect(relicShopHtml).toContain('Recupera reliquias perdidas con una nueva rareza');
    ...
    expect(relicShopHtml).toContain('data-shop-relic-mode="buy"');
    expect(relicShopHtml).toContain('data-shop-relic-mode="sell"');
    expect(relicShopHtml).toContain('RELIQUIAS PERDIDAS');
    expect(relicShopHtml).not.toContain('VENDE TUS RELIQUIAS');
    renderShopView(document, state, 20 * 86400000, { section: 'relics', relicMode: 'sell' });
    const relicSaleHtml = document.elements.shopBody.innerHTML;
    expect(relicSaleHtml).toContain('VENDE TUS RELIQUIAS');
    expect(relicSaleHtml).toContain('RECIBES EL 70% EN ORO');
    expect(relicSaleHtml).toContain('shop-sale-relic-grid');
    expect(relicSaleHtml).toContain('data-open-sale-relic=');
    expect(relicSaleHtml).toContain('shop-sale-relic-price');
    expect(relicSaleHtml).not.toContain('class="shop-relic-sell"');
    expect(relicSaleHtml).toContain('No recuperas Sangre de Jefe');
    expect(relicSaleHtml).not.toContain('RELIQUIAS PERDIDAS');
    expect(renderRelicDetail(document, state, 'relic_01', { shopSale: true })).toBe(true);
    const saleDetailHtml = document.elements.relicDetailBody.innerHTML;
    expect(saleDetailHtml).toContain('EL CONTRABANDISTA PAGA');
    expect(saleDetailHtml).toContain('data-sell-relic="relic_01"');
    expect(saleDetailHtml).not.toContain('data-open-forge-relic');

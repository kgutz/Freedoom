...
import { relicCombatBonus, relicRankEffect } from '../data/loot-data.js';
...
describe('Colmillo: recuperación de vida y nuevas fusiones', () => {
  it.each([[1,5],[2,8],[3,13]])('rango %i conserva fuerza y configura %i%% por victoria sin curación al equipar', (rank, percent) => {
    ...
    expect(day.hpRecovered).toBe(0);
    expect(relicRankEffect('relic_06', rank)).toBe(percent);
  ...
  it('no cura ni resucita por tiempo o cambios de equipo', () => {
    ...
    expect(advancePeriodicHealthRecovery({state:first,nowTimestamp:1000+HALF_HOUR,maxHp:1000,currentHp:999}).hp).toBe(999);
    ...
    expect(resumed.inventory.periodicEffects.healthRecovery).toBeUndefined();
  });
  it.each(['relic_01','relic_02','relic_05'])('migra la fusión con %s sin perder rareza, rango, XP histórica o ingredientes', left => {
    ...
    expect(migrated.inventory.relics[id]).toMatchObject({rank:2,rarity:'legendary',affixes:['vitality'],inheritedEffects:{relic_06:8}});
  ...
  it('Daga hereda ambos ataques y extras sin duplicados; cura una vez al día', () => {
    ...
    expect(state.inventory.relics.fusion_17.inheritedEffects).toEqual({relic_03:2,relic_06:5});
  ...
  it('Frasco y Colmillo fusionados eliminan la recuperación periódica', () => {
    ...
    expect(next.hpRecovered).toBe(0);
